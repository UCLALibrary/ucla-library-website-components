...
// PROPS & DATA
const { nextTo, previousTo, pages, initialCurrentPage, generateLinkCallback } =
    defineProps({
        nextTo: {
            type: String,
            required: false,
...
function generateLink(pageNumber: number) {
    ...
    if (generateLinkCallback) {
        ...
        if (queryParams) return generateLinkCallback(pageNumber, queryParams)
    ...
    if (pages && maxPages) {
        ...
        if (currPage.value > maxPages.value) {
            ...
            if (stop > pages) {
                ...
                if (currPage.value === pages) newMaxPages = newMaxPages + 1 // add another 1 back because 'next' button is hidden
...
function setPaginationMaxPages(width: number) {
    ...
    if (!initialCurrentPage || !pages) return 10
...
onMounted(() => {
    ...
    if (!initialCurrentPage || !pages) return
