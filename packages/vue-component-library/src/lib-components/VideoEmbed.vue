...
/*
Reference: LADI-5244, 5311

Craft injects a generic iframe title 'YouTube video player' into embedded video content that Chrome is unable to override. This becomes an accessibility issue when there are multiple embed videos on a page and they share the same generic title. To resolve this issue in Chrome, we use the oEmbed api to retrieve a video's title from its metadata and assign it to the iframe element's title attribute.
...
if (parsedTrailer.value) {
  ...
  watchEffect(() => {
    ...
    const item = Array.isArray(data.value) ? data.value[0] : data.value

    parsedIframeTitle.value = item?.title ?? 'YouTube Video Player'
  })
}
</script>
