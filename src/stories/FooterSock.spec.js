describe("FOOTER / Sock", () => {
    it("Default", () => {
        ...
        cy.contains("a", "EEEmergency").should("have.attr", "target", "")
        // Opens external link in same tag
        cy.contains("a", "AAAccessibility").should("have.attr", "target", "_blank")
        // Explicitly set to open in same tab
        cy.contains("a", "PPPrivacy & Terms of Use").should("have.attr", "target", "")
        // Explicitly set to open in new tab
        cy.contains("a", "CCCreative Commons Attribution 4.0").should("have.attr", "target", "_blank")
