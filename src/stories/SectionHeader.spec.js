describe("SectionHeader", () => {
    it("defaults to <h1>", () => {
        cy.visit(
            "/iframe.html?id=section-header--default&args=&viewMode=story"
    ...
    it("uses header levels from enclosing <SectionWrapper>s", () => {
        cy.visit(
            "/iframe.html?id=section-header--default&args=&viewMode=story"
        ).get(".section-header").each((element, index) => {
            cy.wrap(element).should("have.class", `section-header${index+1}`)
        })
    })
})
